...
class AgentFunctions(llm.FunctionContext):
    ...
    @llm.ai_callable()
    async def show_transcription(
        self,
        target_language: Annotated[str, llm.TypeInfo(description="The language to translate the transcript to.")],
        transcript: Annotated[str, llm.TypeInfo(description="The transcript to display.")],
    ):
        """Show a transcription to the user."""
        # return f"Section '{section_title}' outlined with key points: {key_points}"
        logger.info(f"🛠️ {self.ctx.room.local_participant.identity} {target_language}: {transcript}")
        payload = {
            "user_id": self.ctx.room.local_participant.identity,
            "target_language": target_language,
            "transcript": transcript
        }

        return await self._call_remote('show_transcription', json.dumps(payload))
